...
function AppleIcon() {
  return (
    <svg width="17" height="17" viewBox="0 0 814 1000" xmlns="http://www.w3.org/2000/svg" fill="white" aria-hidden="true">
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-[14px] font-semibold text-ink disabled:opacity-50 flex items-center justify-center gap-2.5"
      >
      ...
      {appleEnabled && (
        <button
          ...
          className="w-full rounded-xl bg-ink px-4 py-3 text-[14px] font-semibold text-white flex items-center justify-center gap-2.5"
        >
          <AppleIcon /> Continue with Apple
        </button>
      )}
      {message && <p className="rounded-xl bg-amber-50 px-3 py-2 text-[14px] text-amber-900">{message}</p>}
    </div>
  );
}

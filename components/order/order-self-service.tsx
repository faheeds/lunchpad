 ...
 *
 * - Manage order — links to /history where the parent can edit items or cancel.
 * - Cancel & refund — quick-cancel inline, calls /api/orders/{id}/cancel which
 *   runs cancelOrderWithRefund (Stripe refund + status update + email). Only
 *   available before the delivery date's cutoff.
 * - Contact restaurant — mailto: and tel: links rendered as a secondary fallback
 *   when the operator has set those in Settings → General.
 */
      ...
      {cutoffPassed ? (
        <>
          <p style={{ fontSize: 12, color: "#938B78", lineHeight: 1.5 }}>
            ...
            {contactEmail || contactPhone ? "using the links below." : "directly."}
          </p>
        ...
        <>
          {/* Primary: manage order link */}
          <Link
            href="/history"
            style={{
              display: "flex", alignItems: "center", justifyContent: "space-between",
              padding: "11px 14px", borderRadius: 10,
              background: "var(--brand-on-white, #2C4031)", color: "white",
              textDecoration: "none", marginBottom: 8,
            }}
          >
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 1 }}>Manage your order</p>
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.7)" }}>
                View details, edit items, or cancel — all in one place.
              </p>
            </div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginLeft: 8 }}>
          ...
          {(contactEmail || contactPhone) && (
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid #E3DBC6" }}>
              <p style={{ fontSize: 11, color: "#938B78", marginBottom: 6 }}>
                Need extra help? Contact {restaurantName}:
              </p>
                ...
                {contactEmail && (
                  <a
                    ...
                    style={{
                      ...
                      fontSize: 12, color: "#5B5446", textDecoration: "none",
                    }}
                  >
